...
Afterwards, we apply this to a protein binding pocket and estimate a free energy of ligand binding using an end-states approach based on GIST. 
...
For simplicity, prepared and solvated Amber topologies based on 1STP can be downloaded by running
\begin{lstlisting}[style=bash]
...
Since we ran our MD simulation with periodic boundaries, we always need to reimage all molecules to the original box using cpptraj's \inlinecode{autoimage} function. 
...
Indexing with \code{.loc} works by providing a voxel index (either a index number or list of numbers) and optionally a column index (column name or list of names). 
...
We will afterwards introduce advanced analysis workflows which allow you to modify and better visualize the resulting data.
\subsubsection{System Preparation}
...
\begin{figure}
	...
	\caption{Water energy using "norm" data represented as volume in the streptavidin binding pocket overlayed with biotin. 
...
This is expected: the binding of biotin to streptavidin is facilitated through a large number of H-bonds and over polar interactions. 
...
To speed up the calculation, smaller GIST grids could be used by focusing on the binding pocket, or by rotating the solute molecule along its principal axes to fit the cuboid grid more exactly.
However, this should be done \emph{before} the MD simulation, since rotating the trajectory compromises the periodic box information.
...
In the tutorial, this corresponds to the solvation of biotin and streptavidin seperated (initial states) and in complex (end-state). 
...
Note that some columns will be written out both with '\_norm' and '\_dens' suffixes, refereing to normalization by solvent molecule or voxel volume respectively.
