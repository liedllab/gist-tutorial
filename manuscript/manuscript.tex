...
Implicit solvent methods such as MM/PBSA or MM/GBSA can quickly provide results for a large number of structures, but yield inaccurate results when compared to more rigorous explicit solvent methods. 
On the other hand, the free energy of solvation can be calculated in a statistically rigorous way using explicit solvent simulations combined with alchemical methods such as free energy perturbation (FEP) or thermodynamic integration(TI).
...
Two major groups of methods that fall into this group are those based on inhomogeneous solvation theory (IST) \cite{Lazaridis1998} and those based on the Ornstein--Zernike (OZ) equation.
The most important example of OZ-based methods is the reference interaction site model (RISM) as well as its extension to three dimensions (3D-RISM).
Those methods compute the 3D solvent distributions by self-consistent solution of the OZ integral equation to predict solvation thermodynamics.
...
Examples of those methods include grid inhomogeneous solvation theory (GIST) \cite{Nguyen2012,Ramsey2016}, WaterMap, SSTMap \todo{what else?}.
The main limitation of those methods is that they rely on an infinite expansion of the entropy in terms of the density distribution, which is truncated after the first or, sometimes, the second term.
...
It has been shown that GIST can produce comparable accuracy to TI by applying a linear correction factor (0.6 for water) to the first-order entropy as follows.
\begin{equation}
...
Furthermore, an implementation based on the particle mesh Ewald (PME) method was presented, which improves the consistency between GIST and popular molecular dynamics engines. 
...
In addition to a short introduction to the method and the theory behind it, an overview of the various different flavors of GIST methods is provided.
...
We show how to interpret the three-dimensional contributions of solvation free energy in a binding pocket and around the ligand. \todo{use active voice over passive.}
...
Previous experience of GIST is not necessary, but the reader should be familiar with the primary sources on the method outlined in the Theory section. \todo{shouldn't a tutorial be the entry point to a new method?}

The presented analyses are run in Python.
A basic understanding of Python is therefore necessary to follow the tutorial. 
...
\begin{equation}
	...
	\equiv -k_\textit{B} \frac{\rho^\textit{0}}{8\pi^\textit{2}} \int \gro \ln\gro d\textbf{r}d\omega
\end{equation}
\todo{We are inconsistent regarding R/kB}
...
\begin{aligned}
	...
	\Delta E_{ww,k} =& \frac{1}{N_k} \left\langle \sum_i^\textit{solvent,k} \; \sum_{j \neq i}^\textit{solvent} U_{ij}\right\rangle - \left\langle E_{ww}^\textit{bulk}\right\rangle
\end{aligned}
\end{equation}
...
\todo{E$_{LJ}$ seems more intuitive to me, need to check what the GIST literature is using -vah}
\begin{equation}
	E_\text{total} = E_\text{elec} + E_\text{lj}
\end{equation}

In PME-GIST, the system is treated as periodic, and the electrostatic energy is split into a short-range term $E_\text{dir}$, which is calculated in direct space, and a long-range term $E_\text{rec}$, which is calculated in reciprocal space.
...
\end{equation}
\todo{How is E$_{dir}$ treated? Is there a cutoff as well (PME GIST paper seems to indicate so)? If so, we should probably note that. }
...
\begin{equation}
	E_\textit{lj} = E_\textit{lj\_short} +  E_\textit{corr}
\end{equation}
...
After choosing an appropriate reference value, we need to subtract this value from E$_{ww}$.
...
\todo{Eww or E$_{ww}$? Doesn't seem consistent so far. -vah}
...
\todo{The reader is left hanging here - what should be done about the convergence issue? Is that bad or can it be ignored? -vah}
...
\end{lstlisting}
\todo{\$\textbackslash AA\$ should produce an angstrom sign in matplotlib (what a pain to have this printed out in \LaTeX\ tho) -vah}
...
It has been shown in literature \cite{Chen2021}\todo{cite Kraml-Waibl paper} that the total solvation entropy in water is best approximated by 0.6 times the first order entropy.
...
\todo{Table 1 is placed quite akwardly at the moment for me, maybe enforce its position? -vah}
\begin{table}
