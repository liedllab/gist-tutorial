...
Double-counting is accounted in two different ways. The first method uses the water-displacement method. This method is applicable when determining the energetic cost of kicking out a water molecule. For example, when a ligand binds to the protein, water molecules will be displaced from the binding pocket to
the bulk. 
...
gist gridcntr 10.5 20.1 30.1 griddim 30 30 30 gridspacn 0.50 out gist.dat
go
\end{lstlisting}
...
Several dx files are written by default, such as the energies, entropies, number density of oxygen, and number density of hydrogen. All quantities mentioned above is written in the output file (output.dat) and can be used to generate DX files for other quantities.
...
\todo{Should be put a picture here?}
