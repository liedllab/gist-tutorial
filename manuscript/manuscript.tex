...
Setting the reference density $\rho_0$ according to the solvent model used in your simulation is important to get correct results. You will also want to reference the bulk mean solvent-solvent enthalpy $E_{ww}$ correctly, though we will show ways to do this differently in a later section of the tutorial.

Densities of typical water models can be found in the \software{cpptraj} manual.
We also provide the densities for water models commonly used with AMBER forcefields in table \ref{tab:ref_densities}. These densities were calculated from 1$\mu$s MD simulations. Pressure was kept constant at 1 bar with the Berendsen barostat and the water box was prepared with tleap to a size of 100$\cdot$100$\cdot$100\AA\ before pressure equilibration. The temperature was kept constant at 300 K with the Langevin thermostat.
\todo{vah -> Finish these calculations}
\begin{table}[h]
	\caption{Reference density $\rho_0$ and mean water-water energy $E_{ww, norm}$ for various water models from NpT simulations at 300K/1bar.}\label{tab:ref_densities}
	\small
	\begin{tabularx}{\columnwidth}{@{}lrr@{}}
		\toprule
		Water Model       & $\rho_0$ /\AA$^{-3}$ & $E_{ww, norm}$ /$kcal\cdot mol^{-1} \cdot water^{-1}$  \\
		\midrule
		TIP3P    & 0.0329 & -9.533   \\
		TIP3PFB  & 0.0334 & -11.369  \\
		TIP4P    & 0.0332 & -9.856   \\
		TIP4P/Ew & 0.0332 & -11.036  \\
		TIP5P    & 0.0329 & -9.596   \\
		SPC/E    & 0.0333 & -11.123  \\
		OPC      & XXXX   & XXXX     \\
		OPC3     & XXXX   & XXXX     \\
	...
	\end{tabularx}
\end{table}
If your simulation settings diverge significantly from the above setting or for other solvent models not listed either here or in the \software{cpptraj} manual, the density should be computed from a pure solvent simulation beforehand.
...
Therefore, to convert from a \inlinecode{\_norm} quantity $X^k_{norm}$ in voxel k to a \inlinecode{\_dens} quantity $X^k_{dens}$ the following equation is applied:
\begin{equation}
	X^k_{dens} = \frac{X^k_{norm}}{V^k} \cdot \frac{N^k_{solvent}}{N_{frames}} 
\end{equation}
with $V^k$, the volume of voxel $k$, $N^k_{solvent}$, the solvent population in voxel $k$, and $N_{frames}$, the number of simulation frames considered in the GIST analysis. Acoordingly, converting $X^k_{dens}$ to  $X^k_{norm}$ is possible using the following equation:
\begin{equation}
	X^k_{norm} = X^k_{dens} \cdot V^k \cdot \frac{N_{frames}}{N_{frames}N^k_{solvent}} 
\end{equation}
\section{Tutorial}
...
We recommend that you center the biotin molecule in space:
\begin{lstlisting}[style=cpptraj]
	parm solvated.parm7
...
\begin{lstlisting}[style=python]
	import mdtraj as md
	pdb = md.load("biotin-centered.pdb")
	# mdtraj converts to nm.
	# *10 to convert back to Angstrom.
	min_xyz = pdb.xyz[0].min(0) * 10.
	max_xyz = pdb.xyz[0].max(0) * 10.
	print(*list(zip("xyz", min_xyz, max_xyz)), sep="\n")
\end{lstlisting}
...
Running the GIST analysis above, an output data file (gist.dat) containing all calculated quantities for each voxel in the system is produced. 
Additionally as outlined in section \ref{sec:running_GIST}, various quantities are written to OPenDX files for accessible visualisation.

To faciliate an accessible way to interact with your GIST data, we recommend using the gisttools library for python3. Open the GIST output with gisttools:
\begin{lstlisting}[style=python]
...
\begin{lstlisting}[style=python]
	display(gist.loc[0, 'population'])
	display(gist.loc[0])
	display(gist.loc[[0,1], ['x', 'y', 'z']])
	display(gist['dTSsix_norm'].head())
	display(gist[['Eww_norm', 'Esw_norm']].head())
\end{lstlisting}

Gisttools provides access to all quantities listed in the GIST output file.
In addition it calculates the total enthalpy per voxel (Eall) as a sum of Eww and Esw and the free energy of solvation (A) per voxel as a sum of Eall and -dTSsix:
\begin{lstlisting}[style=python]
	display(gist[['A_norm', 'Eall_norm']].head())
\end{lstlisting}

Information about the grid is stored in the grid property of the gist object, structural data is stored as mdtraj trajectory in the struct property:
\begin{lstlisting}[style=python]
	print(gist.grid)
	print(gist.struct)
\end{lstlisting}
...
This tutorial discusses the visualisation of OpenDX files in section \ref{sec:visualisation.}
...
A more sophisticated approach would identify one or several dominant structures out of the ensemble - either gathered from experimental data or by clustering an MD simulation.
...
Note that due to the solute restraints, some important solvent sites might be unavailable to exchange with the bulk solvent.
Make sure to explicitly place water at such sites, if they are important to the process of interest e.g. waters in confined binding pockets, as they might be missed by automated solvation procedures.
It is good practice to check your restrained MD simulation or solvent populations obtained by GIST afterwards to confirm wether all important solvent sites were properly solvated.
...
For larger systems like whole proteins you can usually get away with a smaller buffer region compared to small molecules.
...
rmax = 12
results = { names[gist] : 
	   gist.integrate_around(
	   		cols, rmax, centers[gist]) 
	   for gist in [biotin, strept, compl]}
...
\todo{vah: The cpptraj manual says that GIST PME energies cannot be decomposed into Esw and Eww -> what's true? Seems to conflict with what is written in this section.}
...
\todo{VJ -> include the PyMOL settings somewhere, this is not the default output}
...
\todo{I doubt this is the holo/complex, should be the apo streptavidin, right? -vah´}
...
%In \software{gistpp}, voxels that are within the distance criteria from any ligand heavy atom is considered as part of the region. The resulting file is a binary file and can be visualized using \software{pymol}. \todo{Not GISTPP}
...
\end{equation}
While this is supported by the standard GIST implementation, it is rarely done due to the large size of the $E_{ww,k\;l}$ matrix.
