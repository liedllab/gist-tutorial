...
Even when running the production simulation in NVT, it is advisable to include an NpT equilibration step to adjust the box size beforehand.
Use enough equilibration time to allow the water to relax around your biotin structure.
...
\begin{lstlisting}[style=amber-in]
	...
	/
\end{lstlisting}
This file would run a 100 ns NpT MD simulation while keeping all atoms except for hydrogens and waters restrained. Note that these are very standard MD settings except for the restraints.
...
\todo{write this, show the various GIST properties here: Eww, Esw, S. Show how to calculate A and E with gisttools}
...
This automatically done for the entropy, using the bulk density specified by \inlinecode{refdens}.
...
In \software{gisttools}, this is done automatically, and can be accessed using the \inlinecode{A\_dens} and \inlinecode{A\_norm} data rows. \todo{norm and dens not defined. }
...
Smaller GIST grids could be used by focusing on the binding pocket, or by rotating the molecule by its principal axes to fit the cuboid grid more exactly.
...
In Reference~\cite{Kraml2020}, a GPU implementation of the energy calculation was implemented, which typically increases the speed by 1--2 orders of magnitude.
...
However, PME-GIST is not supported on the GPU.
...
To run a PME-GIST calculation, the \inlinecode{pme} flag to the \inlinecode{gist} action in \software{cpptraj} can be used.
...
For typical use cases, it is recommended to use \inlinecode{Eww} and \inlinecode{Esw} over the \inlinecode{PME} column.
...
To run a GIST calculation in a solvent other than water, use the \inlinecode{solute} flag together with a \software{cpptraj} selection mask.
Everything that is not in the \inlinecode{solute} mask is treated as solvent.
If there are multiple solvent species, use the \inlinecode{solventmols} option as described in Section~\ref{sec-salt-water}.
The code will automatically choose three atoms which are used to define the solvent orientation, and print this selection to the output.

Sometimes, \software{cpptraj} does not automatically choose the optimal atoms.
In methanol, for instance, one can either incorporate the C--H bonds or the O--H bond in the selection.
...
It can be visualized in PyMOL as follows:
...
In this example, the region defined in the binding pocket includes all voxels that are within \SI{3.5}{\angstrom} of the heavy atoms of biotin. 
...
\begin{equation}
	E_\textit{elec} = E_\textit{dir} + E_\textit{rec} + E_\textit{self}
\end{equation}
...
\begin{equation}
	E_\textit{lj} = E_\textit{lj,\ short} +  E_\textit{lj\_corr}
\end{equation}
