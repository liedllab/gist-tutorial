...
For water, this corresponds to a buffer zone of around \qty{10}{\angstrom} around the solute. Integrating GIST energies and energies in a \qty{10}{\angstrom} volume surrounding neutral small molecule solvents is sufficient to estimate solvation free energies \cite{Chen2021,Waibl2022-gist-solvents} however, larger grids might be necessary for strongly charged solutes.
...
The solvent-solvent energy $E_{ww}$ also needs to be referenced, but this is done in the post-processing stage. The calculation of these properties and the need for referencing is discussed in-depth in section \ref{sec:theory}.
...
The next step is to run an MD simulation based on the equilibrated structure of biotin to generate solvent configurations for subsequent GIST analysis. The density expansion in IST relies upon the Percus source particle \cite{lebowitz_percus_1964} which has a rigid solute. Motion of the solute has the effect of smoothing out the water density distributions leading to overestimates of the solvent entropy. The more solute motion, the greater the overestimate. The MD simulations can be run with constraints that prevent the motion of the solute. In practice, however, the solute heavy atoms can be restrained so that the solute does not move significantly during the MD simulation. To minimize the entropy overestimate, we recommend that RMSD of the solute during simulations be \qty{0.1}{\angstrom} or lower.  In AMBER, the harmonic restraint potential takes the form $U = k_r * (r - r_0)^2$ for each cartesian coordinate of each restrained atom, where $r_0$ is its reference position. The force constant $k_r$ should be chosen strong enough to prevent significant deviation. Here, we apply a harmonic restraint of \qty{100}{\kilo\calorie\per\mole\per\angstrom\squared} to all heavy atoms. We recommend of at least \qty{2.5}{\kilo\calorie\per\mole\per\angstrom\squared} restraints. 

The longer the simulations, the better the convergence of GIST energy and entropy estimates with the entropy values converging more slowly. An analysis of the convergence of the entropy in GIST can be found in Ramsey et al. \cite{Ramsey2016}. High precision energy and entropy values are necessary for calculating integrated GIST quantities (see Section \ref{VisualizingFreeEnergy}) as small errors on a per-molecule basis can lead to large errors when summing over hundreds or thousands of water molecules. For qualitative mapping of solvation thermodynamic quantities, however, significantly shorter simulations generally suffice. For integrated quantities, we recommend \qty{100}{\nano\second} trajectories and, for mapping, trajectories of \qtyrange{5}{10}{\nano\second} are generally sufficient. We note that storing solvent configurations too frequently from MD trajectories can lead to systematic bias in the nearest neighbors entropy estimates (see Huggins \cite{Huggins2014-entropy}). We recommend that configurations be stored no more frequently than every \qty{2}{\pico\second}. 
...
# The rdf function allows muktiples columns to be calculated at once
...
Most of the time, this happens due to a systematic bias in the Nearest-Neighbor entropy calculation which converges to the correct value from above \cite{Chen2021,Huggins2014-entropy}.
...
The streptavidin binding pocket is largely hydrophobic leading to energetically unfavorable water in the middle and right of the cavity. The green regions on the left of the cavity are where biotin forms hydrogen bond interactions with the protein (green).
...
\begin{figure}
	...
	\caption{Water energy using "norm" data represented as volume in the streptavidin binding pocket overlayed with biotin. Red: high solvent energy (from +3.5 
	to +2.5 kcal/mol/molecule). Blue: low solvent energy (from -3.5 to -2.5 kcal/mol/molecule). The energy density is based on the apo structure without double-counting and is shown only within \qty{1}{\angstrom} of biotin.}\label{fig_binding_pocket_pymol}
\end{figure}
...
# select the biotin atoms, we multiply by 10 to convert nm to Angstrom.
...
The PV difference between the Helmholtz and Gibbs Free Energies in condensed matter systems  is negligible for condensed phase systems at biological and STP conditions and hence in the literature the terms $\Delta G_{solv}$ and $\Delta A_{solv}$ are often used interchangeably.  
