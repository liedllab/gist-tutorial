...
Cpptrajs' gist action has a large number of options to modify the GIST calculation or its' output. A full list is provided at the end of this tutorial or through cpptrajs manual. The major options are summarized in the following table:
...
Setting the reference density according to the water model used in your simulation is important to get correct results. The reference density can be calculated from a GIST calculation for a pure water simulation or estimated from bulk-like water sufficiently far away from a solvent. \todo{How much information on the choice of reference density should we include?}
