...
The solvent-solvent enthalpy $E_{ww}$ also needs to be referenced, but this is usually done in the post-processing stage.

Densities of typical water models can be found in the \software{cpptraj} manual.
We also provide the densities for water models commonly used with AMBER forcefields in table \ref{tab:ref_densities}.
These densities were calculated from \SI{1}{\micro\second}\pdfcomment{weird small mu symbol} MD simulations at \SI{1}{\bar} and \SI{300}{\kelvin} using the Berendsen barostat and the Langevin thermostat.
The water boxes were prepared with \software{tleap} to a size of \SI{100 x 100 x 100}{\angstrom} before pressure equilibration.
...
\end{equation}
with $V^k$, the volume of voxel $k$, $N^k_{solvent}$, the solvent population in voxel $k$, and $N_{frames}$, the number of simulation frames considered in the GIST analysis.
