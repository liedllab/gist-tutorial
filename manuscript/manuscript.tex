...
The high spatial resolution of the GIST output, as well as the decomposition into energy and entropy contributions, allow for highly detailed analyses of solvation around both proteins and small molecules contexts. However, this versatility also comes with a significant entry barrier for new users.
...
\begin{figure}
    ...
    \caption{Schematic showing regions of high water density around a carbazole solute molecule. The energy is computed from the intermolecular interactions of a force field, while the entropy is approximated from the 3-dimensional solvent distribution around the solute molecule. Note that while this depiction focuses of regions of high solvent density, regions with lower density can also contain meaningful contributions to the energy and entropy.}
    \label{fig:carbazole}
\end{figure}
...
\end{lstlisting}
Additionally, the GIST calculation should usually be preceded by preprocessing steps, such as centering the solute molecule in the unit cell or reimaging to account for periodic boundary conditions.
...
Using \inlinecode{cpptraj's } \inlinecode{bounds} command, the region of interest can be output to a file. For example, here is the command for a region of \SI{10}{\angstrom} around the ligand LIG:
...
The reference values can be calculated from MD trajectories of bulk solvent. From this, one then calculates the density-weighted average energy per solvent molecule ($E_{ww, norm}$) and the average solvent density $\rho_0$.  
...
%TODO we are missing some code here
...
\end{lstlisting}
%TODO: This has a higher indent than the other python snippets in the pdf, somehow. Unsure why...
...
SR and DR programmed cpptraj's GIST implementation and provided technical input.\\  
