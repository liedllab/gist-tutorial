...
The enthalpic contributions are computed from the intermolecular interactions of the solvent with the solute and as well as other solvent. 
...
However, it has been found that solvation free energies of small molecule solutes can be computed at high accuracy using linear scaling factors for the higher-order terms, such as $\Delta S = 0.6\, \Delta S^\text{1st-order}$ for water \cite{Chen2021,Waibl2022-gist-solvents}.
...
Conceptually close to GIST is Grid Cell Theory (GCT), which also adopts a grid-based approach but differs in the assumptions made its' entropy calculations \cite{Gerogiokas2014-GCT}.
...
# Remove the biotin molecule (Nr.2) from the complex, save the topology and coordinates
...
# Calculate the interaction energy of the complex and the monomers
...
Similarly, the entropy columms are named dTS, rather than the more correct $T\Delta S$.
