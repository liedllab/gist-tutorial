...
Additionally, the GIST calculation should usually be preceded by preprocessing steps, such as centering the trajectory or reimaging to account for periodic boundary conditions.
...
In addition, a helper script called \inlinecode{FindCentroid.py} for the same purpose is supplied with this tutorial, and can be used as follows:
...
The reference values can be calculated from MD trajectories of bulk solvent. From this, one then calculates the average energy per solvent molecule ($E_{ww, norm}$) and the average solvent density $\rho_0$.  
Generally, it is recommended that the size of the reference solvent box closely matches that of the production system.
...
Afterwards, we apply this knowledge to a protein binding pocket, visualize $\dgsolv$ hotspots and estimate a free energy of ligand binding using an end-states approach based on GIST. In addition, we discuss all steps shown in the first example in further detail. 
...
nnsearchlayers <val> & Layers of neighboring voxels considered in nearest neighbor search. Higher values may improve entropy convergence for little sampling or fine grid spacings, but increase the calculation time (default 1). \\
