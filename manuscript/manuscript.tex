...
\corr{klaus.liedl@uibk.ac.at}{KRL}  % Correspondence emails.  FMS and FS are the appropriate authors initials.
...
Different from most other methods, these contributions are therefore spatially resolved, improving interpretability and allowing better insights into the nature of solvation around the investigated solute.
...
Nevertheless, solvation free energies of small molecule solutes can be computed at high accuracy using linear scaling factors for the higher-order terms, such as $\Delta S = 0.6\, \Delta S^\text{1st-order}$ for water \cite{Chen2021,Waibl2022-gist-solvents}.
...
Using \inlinecode{cpptraj's } \inlinecode{bounds} command, the region of interest can be output to a file. For example, here is the command for a region of 10\AA{} around the ligand LIG:
...
The solvent-solvent enthalpy $E_{ww}$ also needs to be referenced, but this can be done in the post-processing stage. The calculation of these properties and the need for referencing is discussed in-depth in section /ref{sec:theory}.
...
While the impact on density is not as pronounced, deviations from the specified settings can lead to differences of up to 0.005 kcal/mol in the energy per water molecule ($E_{ww, norm}$) for water boxes. 
...
\begin{table}[h]
	...
	\caption{Reference number density $\rho_0$ and mean water-water energy $E_{ww, norm}$ for various water models from NPT simulations of N$_{WAT}$ water solvent molecules at \SI{300}{\kelvin} and 1bar.}\label{tab:ref_densities}
	\small
...
%TODO we are missing a pymol script here
...
\qquad plcut <val> & Pair list cutoff for order calculation. Default is 10 Å\\
...
Angstroms. Values greater than 0.75 Å are not recommended. Default is 0.5 Å.\\
neighborcut <val> & Cutoff in Å for determining solvent O-O neighbors. Default is 3.5 Å. \\
...
point values in GIST output file. Default chooses 'fixed' or 'scientific' automatically. \\
...
oldnnvolume & Use the old reference volume for the nearest neighbor entropy, instead of the more precise new implementation. \\
...
rigidatoms <central> <atom1> <atom2> & Specifies how to define the molecular orientation for the entropy. 
By default, a simple heuristic will be used. 
This works for water, but not for all solvents. 
The atoms should be representative of the molecular orientation and should not be collinear. 
Note that the central atom goes first. 
For water, the default is equivalent to rigidatoms O H1 H2, corresponding to H1-O-H2 as the rigid substructure. \\
\bottomrule
\end{tabular}  
\end{checklist}
\end{Checklists*}
...
pme & Use particle mesh Ewald for the non-bonded electrostatics calculation. The van der Waals energy will be calculated using a long-range correction for periodicity. Does not support doeij.\\
\qquad cut <val> & Direct space cutoff for pme. Default is 3.5 Å.\\
...
\qquad erfcdx <val> & Spacing to use for the ERFC splines. Default is 0.0002 Å. \\
...
\textbf{Output}\\
GIST calculations put out a variety of data sorted by voxel into a file specified by the "out" keyword. 
...
Esw & X &  Mean solute-solvent interaction energy density. \\
Eww & X & Mean solvent-solvent interaction energy density. \\
PME & [pme] & Solvent PME energy density. \\
U\_PME & [pme] & Solvent PME energy density. \\
dTStrans & X & First order translational entropy density. \\
dTSorient & X & First order orientational entropy density . \\
neighbor & X & Mean number of solvent molecules neighboring the solvent molecules found in this voxel multiplied by the voxel number density. \\
...
dipolex & X & x-component of the mean solvent dipole moment density \\
dipoley & X & y-component of the mean solvent dipole moment density \\
dipolez & X & z-component of the mean solvent dipole moment density \\
Eij & X & (if \inlinecode{doeij} was specified) solvent-solvent interaction matrix. \\
\bottomrule
\end{tabular}
\end{checklist}
\end{Checklists*}
...
MKG has an equity interest in and is a cofounder and scientiﬁc advisor of VeraChem LLC.
